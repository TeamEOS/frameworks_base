...
import com.android.internal.util.actions.ImageHelper;
import com.android.internal.util.gesture.EdgeGesturePosition;
import com.android.internal.util.gesture.EdgeServiceConstants;
import com.android.systemui.R;
import com.android.systemui.statusbar.BaseStatusBar;
import com.android.systemui.statusbar.Hintable;
 ...
 */
public class PieController implements Hintable, PieView.OnExitListener,
    ...
    private final class SettingsObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange, Uri uri) {
            ...
            if (uri.equals(Settings.System.getUriFor(
                ...
                ArrayList<ButtonConfig> buttonsConfig = Config.getConfig(mContext,
                        ActionConstants.getDefaults(ActionConstants.PIE_SECONDARY));
    ...
    public PieController(Context context, EdgeGestureManager pieManager, NavigationBarOverlay nbo) {
        mContext = context;
    ...
    private void setupContainer() {
        if (mPieContainer == null) {
            mPieContainer = new PieView(mContext, mNavigationBarOverlay);
    ...
    private void setupNavigationItems() {
        ...
        buttonsConfig = Config.getConfig(mContext, ActionConstants.getDefaults(ActionConstants.PIE_PRIMARY));
        ...
        if (mSecondLayerActive) {
            ...
            buttonsConfig = Config.getConfig(mContext, ActionConstants.getDefaults(ActionConstants.PIE_SECONDARY));
    ...
    private void getCustomActionsAndConstruct(ContentResolver resolver,
        ...
        if (!secondLayer) {
            mMenuButton = constructItem(1, ActionHandler.SYSTEMUI_TASK_MENU,
                    ActionHandler.SYSTEMUI_TASK_NO_ACTION,
                    ActionConstants.EMPTY,
                    minimumImageSize);
    ...
    private void addItemToLayer(PieSliceContainer layer, ButtonConfig buttonConfig,
        ...
        layer.addItem(constructItem(buttonWidth,
                buttonConfig.getActionConfig(ActionConfig.PRIMARY).getAction(),
                buttonConfig.getActionConfig(ActionConfig.SECOND).getAction(),
                buttonConfig.getActionConfig(ActionConfig.PRIMARY).getIconUri(), minimumImageSize));

        if (buttonConfig.getActionConfig(ActionConfig.PRIMARY).getAction()
                .equals(ActionHandler.SYSTEMUI_TASK_HOME)) {
            layer.addItem(constructItem(buttonWidth,
                    ActionHandler.SYSTEMUI_TASK_ASSIST,
                    buttonConfig.getActionConfig(ActionConfig.SECOND).getAction(),
                    ActionConstants.EMPTY,
                    minimumImageSize));
        }
    ...
    private PieItem constructItem(int width, String clickAction, String longPressAction,
        ...
        if (!longPressAction.equals(ActionHandler.SYSTEMUI_TASK_NO_ACTION)) {
    ...
    private int setPieItemIcon(ImageView view, String iconUri, String clickAction) {
        Drawable d = ActionUtils.getDrawableForAction(mContext, clickAction);
        if (d != null) {
            view.setImageDrawable(d);
        }

        if (iconUri != null && !iconUri.equals(ActionConstants.EMPTY)
            && !iconUri.startsWith(ActionHandler.SYSTEM_PREFIX)) {
            if (clickAction.equals(ActionHandler.SYSTEMUI_TASK_BACK)) {
            ...
            if (clickAction.startsWith(ActionHandler.SYSTEM_PREFIX)) {
                if (clickAction.equals(ActionHandler.SYSTEMUI_TASK_BACK)) {
    ...
    private Drawable resizeIcon(ImageView view, Drawable d, boolean useSystemDimens) {
        ...
        Drawable dResized = ImageHelper.resize(
                mContext, dOriginal, ActionUtils.pxToDp(mContext, (int) (size * mIconResizeFactor)));
        if (d == null) {
    ...
    protected void setNavigationIconHints(int hints, boolean force) {
        ...
        for (int j = 0; j < 2; j++) {
            item = findItem(ActionHandler.SYSTEMUI_TASK_BACK, j);
            if (item != null) {
    ...
    protected void setDisabledFlags(int disabledFlags, boolean force) {
        ...
        for (int j = 0; j < 2; j++) {
            ...
            item = findItem(ActionHandler.SYSTEMUI_TASK_HOME, j);
            if (item != null) {
                item.show(!disableHome);
                // If the homebutton exists we can assume that the keyguard
                // search button exists as well.
                item = findItem(ActionHandler.SYSTEMUI_TASK_ASSIST, j);
                item.show(disableHome);
            }
            item = findItem(ActionHandler.SYSTEMUI_TASK_RECENTS, j);
            if (item != null) {
    ...
    public void onLongClick(PieItem item) {
        ...
        if (!ActionHandler.isActionKeyEvent(type)) {
        ...
        //ActionHandler.performTask(mContext, type, true);
        ActionHandler.performTask(mContext, type);
    }
    ...
    public void onClick(PieItem item) {
        ...
        if (!type.equals(ActionHandler.SYSTEMUI_TASK_MENU)) {
        ...
        //ActionHandler.performTask(mContext, type, false);
        ActionHandler.performTask(mContext, type);
    }
